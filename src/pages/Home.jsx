...
// Hook per a la configuració de la home
function useConfig() {
  return useQuery({
    queryKey: ['config'],
    queryFn: async () => {
  ...
  return (
    <div>
        ...
        <div className="max-w-5xl mx-auto px-6 py-12 md:py-16">
          ...
          <div className="flex flex-wrap gap-8 mt-10 pt-8 border-t border-border">
            {[
              ...
              { n:'67', l:'Municipis coberts',  to:'/pobles' },
      ...
      {bannerActiu && cfg?.banner_text && cfg?.banner_link && (
        <Link to={cfg.banner_link}
          className="block group"
          style={{ background: cfg.banner_color || '#0e2a6e' }}>
          <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className="w-1.5 h-6 bg-white/30 rounded-full flex-shrink-0" />
              <span className="text-white font-semibold text-sm leading-snug">
                {cfg.banner_text}
              </span>
            </div>
            <div className="flex items-center gap-1.5 text-white/70 text-xs font-mono whitespace-nowrap flex-shrink-0 group-hover:text-white transition-colors">
              Veure
      ...
      <div className="bg-white border-b border-border">
        <div className="max-w-5xl mx-auto px-6 py-10">
          ...
          {postsLoading ? (
            ...
            <div className="text-sm text-mid text-center py-8">
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            { to:'/pobles',         icon:'🏠', label:'Pobles',                 desc:'67 ajuntaments' },
            ...
            { to:'/',               icon:'⚔️', label:'PP vs PSOE',             desc:'Comparativa temàtica' },
    ...
    <Link to={`/blog/${p.slug}`}
      ...
      {p.imatge_url ? (
        ...
        <div className="aspect-video flex items-center justify-center"
          style={{ background: cat?.color ? `${cat.color}15` : '#f0ede8' }}>
          <svg className="w-8 h-8 opacity-20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1">
      ...
      <div className="p-4 flex-1 flex flex-col">
        {cat && (
          <span className="font-mono text-[9px] font-bold tracking-[1px] uppercase mb-1.5"
            style={{ color: cat.color }}>{cat.nom}</span>
        )}
        <h3 className="font-display font-bold text-sm text-ink leading-tight group-hover:text-accent transition-colors flex-1 line-clamp-3">
          {p.titol}
        </h3>
